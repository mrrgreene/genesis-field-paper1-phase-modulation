# Updated mcmc_fixedM_upgraded.py with requested changes
...
df = df[df['z'] > 0.023]  # Remove only low-z, keep all sigma_mu
...
# === Luminosity Distance (using trapezoid method) ===
def dL_ripple(zarr, Om, eps, omega, phi, gamma, H0):
    # Pre-sample a grid in z
    zp_grid = np.linspace(0, np.max(zarr), 5000)
    Hz_grid = ripple_Hz(zp_grid, Om, eps, omega, phi, gamma, H0)
    integrand = c / Hz_grid
    integral_grid = cumulative_trapezoid(integrand, zp_grid, initial=0)
    # Interpolate result at actual z values
    dL_vals = np.interp(zarr, zp_grid, integral_grid)
    return (1 + zarr) * dL_vals  # in Mpc

# === Distance Modulus Model ===
def mu_model(zarr, Om, eps, omega, phi, gamma, H0):
    dL = dL_ripple(zarr, Om, eps, omega, phi, gamma, H0)
...
if __name__ == "__main__":
    ...
    edge_flags = {}
    for name, (lo, hi), val in zip(labels, 
        [(0.05, 0.5), (-0.01, 0.01), (0.01, 0.3), (-np.pi, np.pi), (0.01, 0.3), (60.0, 75.0)], 
        best_fit):
    ...
    summary = {
        "M_locked": M_locked,
        "best_fit": dict(zip(param_names, best_fit)),
        "uncertainty": dict(zip(param_names, uncertainty)),
        "chi2": chi2,
        "aic": aic,
        "bic": bic,
        "chi2_dof": chi2 / (n - k),
    ...
    print("\n✅ Saved sn_mcmc_pantheon_summary.json with diagnostics")
