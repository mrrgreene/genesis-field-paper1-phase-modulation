# Genesis Field H(z) Tight Fit — Pantheon-Calibrated
import numpy as np
import pandas as pd
...
# === Step 1: Load and Combine Observational Data ===
bao_df = pd.DataFrame({
...
# === Step 2: Ripple model with Pantheon-Calibrated Ωₘ ===
Om_fixed = 0.36711  # Updated from full Pantheon MCMC (no σ cut)
...
# === Step 3: Likelihood and Pantheon-Calibrated Priors ===
...
initial = [0.00003, 0.1547, -0.0187, 0.1548, 70.24]  # Pantheon-based seed
pos = initial + 1e-4 * np.random.randn(nwalkers, ndim)

sampler = emcee.EnsembleSampler(nwalkers, ndim, log_prob)
print("Running Genesis Field MCMC (Tight Fit, Pantheon-calibrated)...")
...
np.save("hz_chain_mcmc_tight.npy", sampler.get_chain())
np.save("hz_log_prob_mcmc_tight.npy", sampler.get_log_prob())
